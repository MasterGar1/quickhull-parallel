"""
ThreadPool class that manages a pool of worker threads and a Future class for handling asynchronous results.
"""
...
class ThreadPool:
    ...
    def __init__(self, num_threads: int):
        ...
        for _ in range(num_threads):
            t = Thread(target=_function, args=(self.tasks, self.res_queue), daemon=True)
            t.start()
...
def _function(task_queue: Queue, res_queue: Queue) -> None:
